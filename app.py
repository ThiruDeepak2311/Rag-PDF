...
def summarize_with_cohere(text):
    response = cohere_client.summarize(
        text=text,
        length="short",  # Adjusted for concise summaries
        format="paragraph",
    )
...
def answer_with_cohere(context, question):
    response = cohere_client.generate(
        model="command-xlarge-nightly",  # Ensure this model is accessible
...
if pdf_file:
    ...
    for entry in st.session_state.chat_history:
        ...
        <div style='background-color: #f9f9f9; padding: 10px; border-radius: 8px; margin-bottom: 10px;'>
            <div style='text-align: right; font-weight: bold; color: #333;'>Q: {entry['question']}</div>
            <div style='text-align: left; font-style: italic; color: #555;'>A: {entry['answer']}</div>
        </div>
    ...
    if st.session_state.show_end_chat:
        if st.button("End Chat"):
            ...
            <div style='text-align: center;'>
                <h3 style='color: green;'>Thank you! The chat has been ended. 🌟</h3>
                <p>Feel free to upload another file to start a new session.</p>
            </div>
            """, unsafe_allow_html=True)
